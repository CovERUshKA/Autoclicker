...
#define CustomMessageBox(title, msg) MessageBoxA(NULL, msg, "Autoclicker - " title, MB_OK | MB_TOPMOST);
#define ErrorMessageBox(msg) MessageBoxA(NULL, msg, "Autoclicker - Error", MB_OK | MB_TOPMOST | MB_ICONERROR);
...
inline LONGLONG GetCurrentTimeMilliseconds()
{
	LARGE_INTEGER t;
	LARGE_INTEGER f;
...
inline bool GetProcAddressEx(HANDLE hTargetProc, HINSTANCE hModule, const wchar_t* szProcName, void*& pOut);

inline bool GetProcAddressEx(HANDLE hTargetProc, const wchar_t* szModuleName, const wchar_t* szProcName, void*& pOut)
{
...
inline bool GetProcAddressEx(HANDLE hTargetProc, HINSTANCE hModule, const wchar_t* szProcName, void*& pOut)
{
	...
	{
		wchar_t pFullExport[MAX_PATH]{ 0 };
		size_t len_out = 0;
		...
		wchar_t* pFuncName = wcschr(pFullExport, L'.');
		*(pFuncName++) = '\0';
		if (*pFuncName == '#')
			pFuncName = ReCa<wchar_t*>(LOWORD(_wtoi(++pFuncName)));
	...
	while (min <= max)
	{
		...
		/*wchar_t* szName = ReCa<char*>(localBase + CurrNameRVA);

		int cmp = wcscmp(szName, szProcName);
		if (cmp < 0)
			min = mid + 1;
		else if (cmp > 0)
			max = mid - 1;
